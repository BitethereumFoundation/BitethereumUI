...
class AccountActions {
    ...
    importAirdrop(account, signature, type){

        var sigBuffer = null;
        if(type === 1){
            sigBuffer = new Buffer(signature, "hex");
        }else{
            sigBuffer = new Buffer(signature, "base64");
        }
        var tr = WalletApi.new_transaction();
        tr.add_type_operation("airdrop_balance_claim", {
            "fee": {
                "amount": 0,
                "asset_id": 0
            },
            "account_to_deposit": account.get("id"),
            "signature": sigBuffer,
            "address_type": type
        });
        return WalletDb.process_transaction(tr, null, true);
