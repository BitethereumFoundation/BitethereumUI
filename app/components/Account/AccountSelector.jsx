...
class AccountSelector extends React.Component {
    ...
    render() {
        ...
        let inputStyle = !this.props.disabled ? {width: 100, height: 34, fontSize: 14, textTransform: "lowercase", color: "#4d4d4d", background: "#ffffff", border: "1px solid #e0e0e0", borderRight: "none"} :
            {width: 100, height: 34, fontSize: 14,textTransform: "lowercase", background: "#f5f5f5", color: "#808080",border: "1px solid #e0e0e0", borderRight: "none"};
        let idTipStyle = !this.props.disabled ? {textAlign: "right", width: 140, display: "inline-block", lineHeight: "32px", fontSize: 14, color: "#4d4d4d", background: "#ffffff", border: "1px solid #e0e0e0", borderLeft: "none", paddingRight: 11} : {textAlign: "right", width: 140, display: "inline-block", lineHeight: "32px", fontSize: 14, color: "#808080", background: "#f5f5f5", border: "1px solid #e0e0e0", borderLeft: "none", paddingRight: 11};

        return (
            <div className="account-selector" style={this.props.style}>
                <div className="content-area">
                    <div className="input-area">
                        <div className="inline-label input-wrapper" style={{paddingLeft: 21, paddingRight: 31}}>
                            <span style={{display: "inline-block", width: 60, fontSize: 14, color: "#4d4d4d", lineHeight: "34px"}}>{this.props.label}</span>
                                <input
                                    ...
                                    type="text"
                                    name="username"
                                    id={"username" + this.props.tabIndex}
                                ...
                                />
                            <span style={idTipStyle}>{this.props.idTip}</span>
                                { this.props.children }
                                { this.props.onAction ? (
                            ...
                            {error ? <div className="error-area">
                                <span style={{marginLeft: 25, display: "inline-block", lineHeight: "20px", fontSize: 12}}>{error}</span>
                            </div> : null}
                        </div>
            </div>
        );
