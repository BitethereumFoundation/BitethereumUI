...
class AccountVoting extends React.Component {
    ...
    onProxyAccountInput(input){
        if(this.state.proxy_account_id && this.state.proxy_account_name !== input){
            this.setState({
                proxy_account_id: ""
            });
        }
    ...
    render() {
        ...
        let publish_buttons_class = cnames("no-padding button action-button", {disabled : !this.isChanged()});

        let {globalObject, dynamicGlobal} = this.props;

        let allActiveWitnesses = globalObject.get("active_witnesses").map(a => {
        ...
        let allActiveCommitteeMembers = globalObject.get("active_committee_members").map(a => {
            let object = ChainStore.getObject(a);
        ...
        let actionButtons = <div className="content-block">
            <button className={cnames(publish_buttons_class, {success: this.isChanged.bind(this)})} onClick={this.onPublish.bind(this)} tabIndex={4}>
                <Translate content="account.votes.publish"/>
            </button>
            <button className={"button outline " + publish_buttons_class} onClick={this.onReset.bind(this)} tabIndex={8}>
                <Translate content="account.perm.reset"/>
            </button>
        </div>
        return (
            <div className="grid-content no-margin no-padding" style={{marginTop: "-7px"}}>
                <Tabs setting="votingTab" tabsClass="no-padding bordered-header" contentClass="grid-content no-padding">
                    <Tab title="explorer.committee_members.title">
                        <div className={cnames("content-block", {disabled : proxy_is_set})}>
                            <HelpContent path="components/AccountVotingCommittee" style={{fontSize: 12}}/>
                            <AccountsList
                                type="committee"
                                label="account.votes.add_committee_label"
                                allItems = {Immutable.List(allActiveCommitteeMembers)}
                                activeItems = {this.state.committee}
                    ...
                    <Tab title="explorer.witnesses.title">
                        <div className={cnames("content-block", {disabled : proxy_is_set})}>
                            ...
                            <AccountsList
                                type="witness"
                                label="account.votes.add_witness_label"
                                allItems={Immutable.List(allActiveWitnesses)}
                                activeItems={this.state.witnesses}
                    ...
                    <Tab title="account.votes.proxy_short">
                        <div className="content-block">
                            ...
                            <AccountVotingProxy
                                ...
                                onAccountInputChanged = {this.onProxyAccountInput.bind(this)}
                                actionButtons = {actionButtons}
                            />
                        </div>
                    </Tab>
